...
class EpicGamesStoreLauncher(Flox):
    ...
    def query(self, query):
        score_threshold = int(self.settings.get('score_threshold', 10))
        try:
            games = egs.get_games()
            for game in games:
                score = match(query, game.display_name)
                show_on_empty_query = self.settings.get('show_on_empty_query', True)
                if (score > score_threshold or (show_on_empty_query and query == '')) and not game.b_is_incomplete_install:
                    self.add_item(
